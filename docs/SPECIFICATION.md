...
- Логика: валидация → запись в `raw_samples` → (опц.) обновление инкрементального дневного
  агрегата для сегодняшнего дня. Никаких сообщений юзеру при нормальном потоке.
...
| `daily-morning`       | `0 7 * * *` (UTC) | Для каждого юзера: если локальное время ≈ morning slot (±окно) → напоминалка «внести ужин/взвеситься» |
| `daily-midday`        | `0 12 * * *`      | Аналогично для midday slot                              |
| `daily-evening`       | `0 20 * * *`      | Аналогично для evening slot (итог дня по калориям/шагам)|
| `workout-reminder`    | `0 * * * *`       | Почасовой джоб: для каждого юзера сверяет `reminder_settings.workout_times` с текущим локальным временем → напоминалка о тренировке |
| `anomaly-check`       | `*/30 * * * *`    | Для каждого юзера: проверка порогов аномалий → алерт   |
...
**Окно совпадения времени (fuzzy):** daily-напоминалки и workout-reminder срабатывают, если
текущая локальная минута юзера попадает в окно `[slot, slot + окно)`; окно = 30 мин на v1,
чтобы компенсировать, что статичный cron ходит раз в час/день. Альтернатива для точности —
динамический schedule-store (`patterns/dynamic-scheduling.md`) на фазе масштабирования.
...
- **План:** недельная структура (дни → упражнения → подходы/повторы) в `workout_programs.plan`.
- **Адаптация:** при пропусках/`partial`/`skipped` бот может перенести тренировку, облегчить,
  пересобрать план (новая `version`). Решение принимает агент на основе логов.
- **Напоминания:** per-user cron через dispatcher (см. §9).
...
типа на юзер×день, чтобы не спамить). Пороги (настраиваются в коде):
- **Сон:** < N часов (напр. < 5ч) или bedtime позже порога (напр. > 02:00).
- **Калории:** текущий дневной калораж уже > целевого + 25% и день не окончен.
- **Активность:** резкое падение шагов vs скользящее среднее.
- **Вес:** резкий скачок (вода и т.п.) — информационно, не тревожно.
Тон алерта — по пресету юзера.
...
### 12.3 Хранение и агрегация (защита от race-condition)
- **Сырые сэмплы (`raw_samples`):** TTL 30 дней, затем схлопывание в `daily_aggregates`
  (schedule `aggregate-raw`) и удаление сырых.
- **Алгоритм `aggregate-raw` (без потерь при гонках):**
  1. На старте джоба фиксируется `cutoff = now() - interval '30 days'` (snapshot).
  2. Для каждого `(user_id, day, metric)` где `recorded_at < cutoff`: вычислить агрегат и
     **upsert** в `daily_aggregates` (если агрегат уже есть — перезаписать последней версией).
  3. Удалить только сэмплы с `recorded_at < cutoff` (на момент snapshot). Новые сэмплы,
     пришедшие во время джоба, останутся и будут обработаны следующим запуском.
  4. Гранулярность транзакции: per `(user_id, day, metric)` — сбой одного дня не роняет весь джоб.
- **upsert агрегата при поздних сэмплах:** если поздний сэмпл (напр. 35-дневной давности,
  пришедший после офлайна) относится к дню, уже агрегированному — `aggregate-raw` перевычислит
  агрегат (upsert) и удалит этот сэмпл. Исторический агрегат обновляется.
