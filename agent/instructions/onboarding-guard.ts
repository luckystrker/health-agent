...
Шаг 7 — выдача токена часов (rotate-phone-hub-token); шаг 8 — подключение
FatSecret через PIN-flow (connect-fatsecret → PIN → complete-fatsecret). Оба
можно пропустить по желанию пользователя (подключаются позже).`,
      });
    },
  },
});
